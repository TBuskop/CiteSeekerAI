...
# --- Query ---
QUERY = """
    I need a summary of the literature on inland & riverine compond floods, its relation to climate change. And also find evidence on differnt types of compound floods, such as rain on snow events, etc.
"""
QUERY_DECOMPOSITION_NR = 3  # Number of sub-queries to generate from the main query

# --- Scopus Search string ---
SCOPUS_SEARCH_STRING = """
    (( "compound event" AND "climate change" AND "flood" ) OR "compound flood modelling" OR ( "flood" AND "climate change" AND "river" ) OR ( "flood" AND "bavaria" AND "climate change" ))    
"""
...
TOP_K_ABSTRACTS = 40  # Number of papers to retrieve based on abstract similarity
...
SUBQUERY_MODEL = "gemini-2.5-flash-preview-05-06"  # Model for generating sub-queries (more powerful is better and relevant questions)
...
DEFAULT_CONTEXT_LENGTH = (
    120  # Desired length of context extracted from chunks (e.g., for summaries)
)
DEFAULT_TOTAL_CONTEXT_WINDOW = (
    2000  # Total context window size for models (informational)
)
...
DEFAULT_CHROMA_COLLECTION_NAME = (
    "rag_chunks_hybrid_default"  # Default collection name in ChromaDB
)
